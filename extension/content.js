...
chrome.storage.sync.get('highlights', (data) => {
    try {
        if (data && data.highlights) {
            // Handle case where highlights might be a string (JSON)
            let highlightsData = data.highlights;
            ...
            // Ensure it's an object and has the URL key
            if (typeof highlightsData === 'object' && highlightsData !== null && !Array.isArray(highlightsData)) {
                highlights = highlightsData[window.location.href] || {};
            } else {
                highlights = {};
            }
        } else {
