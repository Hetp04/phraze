...
export const IconConstellation: React.FC<IconConstellationProps> = ({ progress }) => {
  ...
  const [glideProgress, setGlideProgress] = useState(progress);

  useEffect(() => {
    let rafId = 0;
    const tick = () => {
      setGlideProgress((prev) => {
        const alpha = 0.085;
        return prev + (progress - prev) * alpha;
      });
  ...
  const dispersalT = remap(glideProgress, 0, 0.55, 0, 1);
  const dispersal = dispersalT * dispersalT * (3 - 2 * dispersalT); // eased movement factor
  ...
  return (
    <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
      {ICONS_CONFIG.map((icon, index) => {
        ...
        const rotation = remap(glideProgress, 0.05, 0.6, 0, rotationDirection * rotationMax);
